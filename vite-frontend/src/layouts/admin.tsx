...
export default function AdminLayout({
  ...
  const managementMenuItems = filteredMenuItems
    ...
    .sort((a, b) => {
      const order = ['/admin/dashboard', '/config'];
  ...
  return (
      ...
      <aside className={`
        ...
        shadow-none
        border-r border-gray-200 dark:border-[#292b2f]
        z-50 
      ...
      `}>
                 {/* Logo 区域 */}
         <div className="px-4 py-3 h-14 flex items-center border-b border-gray-200 dark:border-[#292b2f]">
         ...
         <nav className="flex-1 px-3 py-4 overflow-y-auto">
           <ul className="space-y-1">
              ...
              return (
                <li key={item.path}>
                     ...
                     className={`
                       ...
                       ${isActive 
                         ? 'bg-primary-100 dark:bg-primary-500/15 text-primary-600 dark:text-primary-300'
            ...
            {managementMenuItems.length > 0 && (
              <li className="pt-3 mt-3 border-t border-gray-200 dark:border-[#292b2f]">
                <button
                  type="button"
                  onClick={() => setManagementMenuOpen(open => !open)}
                  aria-expanded={managementMenuOpen}
                  className="w-full flex items-center justify-between gap-3 px-3 py-2 rounded-md text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/[0.04] transition-colors"
                >
                  ...
                  <svg className={`w-4 h-4 transition-transform duration-200 ${managementMenuOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="m6 9 6 6 6-6" />
                  </svg>
                </button>

                {managementMenuOpen && (
                  <ul className="mt-1 ml-3 pl-3 space-y-1 border-l border-gray-200 dark:border-[#33353a]">
                    {managementMenuItems.map((item) => {
                      ...
                      return (
                        <li key={item.path}>
                          <button
                            onClick={() => handleMenuClick(item.path)}
                            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-left transition-colors duration-200 min-h-[40px] ${isActive
                              ? 'bg-primary-100 dark:bg-primary-500/15 text-primary-600 dark:text-primary-300'
                              : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/[0.04]'}`}
                          >
        ...
        <div className="px-4 py-2 pb-4 mt-auto flex-shrink-0">
          <div className="text-center">
            <p className="text-xs text-gray-400 dark:text-gray-500">
              Powered by{' '}
              <a 
                href="https://github.com/wyusgw/flux-panel" 
                target="_blank" 
      ...
      <div className={`flex flex-col flex-1 ${isMobile ? 'min-h-0' : 'h-full overflow-hidden'}`}>
                 {/* 顶部导航栏 */}
         <header className="bg-white dark:bg-[#111214] shadow-none border-b border-gray-200 dark:border-[#292b2f] h-14 flex items-center justify-between px-4 lg:px-6 relative z-10">
        ...
        {/* 主内容 */}
        <main className={`flex-1 bg-gray-100 dark:bg-[#0c0d0f] ${isMobile ? '' : 'overflow-y-auto'}`}>
          {children}
        </main>
      </div>
